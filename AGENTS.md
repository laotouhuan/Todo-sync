...
1. **修改 Android 代码后必须运行测试**：执行 `./gradlew lintDebug testDebugUnitTest`，全部通过后才可提交。
...
该脚本会自动检测 Windows 端和 Android 端的配置，并在单次运行中跑完所有测试门禁。
...
## 13. ⚠️ Android 编译环境与命令

在本地开发或通过终端沙盒环境执行 Android 编译时，若遇到 `JAVA_HOME is not set` 错误，说明缺少全局 JDK 环境。

### 解决方案（AI 助手专用调用模板）
为了在执行 `./gradlew` 系列命令时畅通无阻，应使用以下 PowerShell 脚本自动搜索并使用 Android Studio 自带的 JDK。后续调用时直接复制并替换最后一条 Gradle 命令即可。
...
./gradlew check
```
> **Note to AI Agents**: 无论何时需要执行 Android 的构建、测试或检查任务，如果发生 Java 环境报错，请务必直接使用这段代码套壳调用 `./gradlew`。
